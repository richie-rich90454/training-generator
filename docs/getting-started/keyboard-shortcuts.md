...
description: Keyboard navigation and shortcuts for Training Generator.
outline: [2, 3]
---

# Keyboard Shortcuts

Training Generator supports keyboard navigation and shortcuts for the most common actions. On macOS, use **Cmd** where Windows and Linux use **Ctrl**.

::: tip Focus-aware shortcuts
Global shortcuts are ignored while an `<input>`, `<textarea>`, or `<select>` element is focused so you can type normally.
:::
...
| `Ctrl/Cmd + K` | Anywhere except text inputs | Open the command palette. |
...
- [User Interface Walkthrough](/getting-started/quick-start.md) — learn the main window layout.
