class TrainGeneratorApp{
    ...
    async processFile(fileObj,progressCallback){
        try{
            ...
            for(let i=0;i<chunks.length;i++){
                let chunk=chunks[i];
                let prompt=this.generatePrompt(chunk,processingType);
                try{
    ...
    generatePrompt(text, processingType) {
        let prompts={
            instruction: `You are an AI training data generator. Your task is to extract comprehensive question-answer pairs from the provided text that cover ALL important information for instruction tuning, supporting both English and Chinese content.
    TEXT TO ANALYZE:
    ${text}
    INSTRUCTIONS:
    1. Read the text thoroughly and identify ALL key concepts, facts, arguments, data points, and important information.
    2. For EACH significant piece of information, create a clear, specific question that someone might ask about it. Questions and answers should be in the same language as the source text. If the text is bilingual, generate pairs in both languages.
    ...
    - Factual questions (who, what, when, where / 谁, 什么, 什么时候, 哪里)
    - Conceptual questions (how, why, explain / 如何, 为什么, 解释)
    - Analytical questions (compare, contrast, analyze / 比较, 对比, 分析)
    - Application questions (how to use, implement, apply / 如何使用, 实施, 应用)
    - Inference questions (what can be concluded, implied / 可以得出什么结论, 暗示)
    ...
    [Continue with as many pairs as needed to cover all information]`,
            conversation: `You are an AI training data generator. Your task is to create comprehensive, informative conversations between a user and an AI assistant based on ALL information in the provided text, supporting both English and Chinese content.
    TEXT TO ANALYZE:
    ${text}
    INSTRUCTIONS:
    1. Read the text thoroughly and identify ALL main topics, key information, arguments, and details.
    2. Create a comprehensive conversation where the user asks questions or discusses ALL important aspects of the text. Questions and responses should match the source text language; if bilingual, include both languages.
    ...
    [Continue with as many exchanges as needed to cover all information]`,
            chunking: `You are an AI training data generator. Your task is to create a comprehensive, detailed summary of the provided text that captures ALL essential information, supporting both English and Chinese content.
    TEXT TO SUMMARIZE:
    ${text}
    INSTRUCTIONS:
    ...
    4. Include both English and Chinese content if present in the text, preserving meaning in each language.
    5. Write in clear, professional language.
    6. Do not add any information not present in the original text.
    7. Do not omit any significant information from the original text.
    8. Include ALL key examples, evidence, and supporting points mentioned.
    OUTPUT FORMAT:
    Provide only the summary text.`,
            custom: `You are an AI training data generator. Your task is to analyze the provided text and extract COMPREHENSIVE structured information that can be used for AI training, supporting both English and Chinese content.
    TEXT TO ANALYZE:
    ${text}
    INSTRUCTIONS:
    ...
    3. Ensure bilingual content is preserved and structured properly if the text contains both English and Chinese.
    4. Focus on extracting COMPLETE, factual, verifiable information from the text.
    5. If the text contains instructions or procedures, extract ALL steps in detail.
    6. If the text contains comparisons or contrasts, highlight ALL key differences and similarities.
    7. If the text contains lists, extract ALL items with their descriptions.
    8. Format your analysis in a clear, organized, comprehensive manner that captures EVERYTHING important from the text.
    OUTPUT FORMAT:
