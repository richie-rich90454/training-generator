  ...
  async function processFiles(): Promise<void> {
    ...
    const pipelineEvents: PipelineEvents = {
      ...
      onChunkProcessed: (event) => {
        ...
        // Use event.fileName directly so concurrent workers (maxParallelFiles > 1)
        // don't clobber a shared currentSourceFile variable.
        outputStore.stageItems(event.items, event.fileName || undefined)
      ...
      onFileComplete: (event) => {
        if (event.success) {
          outputStore.appendOutput(event.items, event.fileName || undefined)
          successfulFiles++
