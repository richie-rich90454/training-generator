...
import { createOrchestrator } from "../processing/orchestrator.js"
...
export function createAppStore(): AppStore {
    ...
    const orchestrator = createOrchestrator({
        processor,
        promptManager,
        createTrainingItem: (input: string, output: string, type: string, format: string) => outputStore.createTrainingItem(input, output, type, format)
    })
    ...
    function updateOutputPreview(): void {
        uiStore.updateOutputPreviewDebounced(outputStore.previewText(), outputStore.itemCount() === 0 && isProcessing())
    }
    ...
    async function processFiles(): Promise<void> {
        ...
        addLog(t("log.startingProcessing", undefined, { count: String(fileStore.fileCount()) }), "info")
        ...
        try {
            ...
            const summaryMessage = successfulFiles > 0
                ? t("toast.processingSummarySuccess", undefined, { successful: String(successfulFiles), total: String(queue.length), count: String(totalItemsGenerated) })
                : t("toast.processingSummaryWarning", undefined, { successful: String(successfulFiles), total: String(queue.length), failed: String(failedFiles), count: String(totalItemsGenerated) })
            ...
            audit.record("processing_completed", { itemsGenerated: totalItemsGenerated, successfulFiles, failedFiles })
            ...
            if (outputStore.itemCount() > 0) {
                ...
                addLog(t("log.outputReady", undefined, { count: String(outputStore.itemCount()) }), "success")
            }
        }
    }
    ...
    async function exportOutput(format?: string): Promise<void> {
        ...
        audit.record("export_triggered", { format: format || outputStore.exportFormat(), itemCount: outputStore.itemCount() })
        await outputStore.exportOutput(format)
    }
    async function copyOutput(): Promise<void> {
        await outputStore.copyOutput()
    }
    ...
    async function loadCheckpointState(): Promise<void> {
        try {
            ...
            if (checkpoint && checkpoint.outputData && checkpoint.outputData.length > 0) {
                const confirmed = await showConfirm(t("confirm.resumeCheckpoint", undefined, { count: String(checkpoint.outputData.length) }))
                if (confirmed) {
                    outputStore.appendOutput(checkpoint.outputData)
                    addLog(t("log.checkpointRestored", undefined, { items: String(checkpoint.outputData.length), files: String(checkpoint.files.length) }), "success")
        ...
        try {
            ...
            fileInput.addEventListener("change", async () => {
                ...
                try {
                    ...
                    const prompt = await promptManager.getPromptWithFallback(language, processingType) || ""
                    const warmed = await warmCache(items as any, model, prompt)
                    uiStore.showToast(t("toast.cacheWarmed", undefined, { warmed: String(warmed), total: String(items.length) }), "success" as ToastType)
                    addLog(t("log.cacheWarmed", undefined, { warmed: String(warmed) }), "success")
                }
                catch (err) {
                    uiStore.showToast(t("toast.readFileFailed", undefined, { error: (err as Error).message }), "error" as ToastType)
                }
            })
            fileInput.click()
        }
        catch (err) {
            uiStore.showToast(t("toast.warmCacheFailed", undefined, { error: (err as Error).message }), "error" as ToastType)
        }
    }
    function showStats(): void {
        ...
        addLog(t("stats.processingComplete", undefined, {
    ...
    async function openUserGuide(): Promise<void> {
        try {
            if (window.electronAPI && window.electronAPI.openUserGuide) {
                ...
                if (!result.success) {
                    addLog(t("log.openUserGuideFailed", undefined, { error: result.error || "" }), "error")
                }
                else {
            ...
            addLog(t("log.openUserGuideFailed", undefined, { error: (error as Error).message }), "error")
        }
    }
    async function init(): Promise<void> {
        ...
        window.addEventListener("error", (event) => {
            logger.error("app", t("log.unhandledError"), { message: event.message, filename: event.filename, lineno: event.lineno })
        })
    ...
    return {
        ...
        uiStore,
        processor,
        get providerManager() { return providerManager() },
        promptManager,
        logger,
        audit,
        isProcessing,
